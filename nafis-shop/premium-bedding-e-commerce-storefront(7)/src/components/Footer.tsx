...
import { Phone, MapPin, Smartphone, CheckCircle2 } from "lucide-react";
...
export function Footer() {
  ...
  const handleNewsletter = async () => {
    if (!newsPhone.trim() || newsPhone.trim().length < 10) return;
    try {
      ...
      setNewsDone(true);
      setNewsPhone("");
      setTimeout(() => setNewsDone(false), 4000);
    } catch {}
  };
  return (
    <footer className="bg-[#3B3228] text-[#EBE4D8] pt-12 pb-8">
      <div className="max-w-[1400px] mx-auto px-4 grid grid-cols-1 md:grid-cols-4 gap-8 pb-8 border-b border-white/10">
        ...
        <div>
          ...
          <ul className="space-y-2 text-xs text-[#8C8175]">
            ...
            <li><Link href="/shop?category=organizers-bedding" className="hover:text-[#9AAF8E] transition">نظم‌دهنده</Link></li>
            <li><Link href="/shop?category=kitchen-table" className="hover:text-[#9AAF8E] transition">آشپزخانه</Link></li>
            <li><Link href="/shop?category=rugs" className="hover:text-[#9AAF8E] transition">روفرشی و فرش</Link></li>
            <li><Link href="/shop?category=towels-main" className="hover:text-[#9AAF8E] transition">حمام و حوله</Link></li>
          </ul>
        </div>
        <div>
        ...
        <div>
          ...
          {newsDone ? (
            <div className="flex items-center gap-2 text-[#9AAF8E] text-xs font-bold">
              <CheckCircle2 className="w-4 h-4" /> ثبت شد!
            </div>
          ) : (
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-white/10 rounded-xl px-3 py-2 text-xs text-white placeholder:text-[#8C8175] focus:outline-none focus:ring-1 focus:ring-[#7A8E72] dir-ltr text-center"
              />
              <button
                onClick={handleNewsletter}
                className="bg-[#7A8E72] hover:bg-[#5C6F54] text-white px-4 py-2 rounded-xl text-xs font-bold transition-all hover:scale-105"
              >
                ثبت
              </button>
            </div>
      ...
      <div className="max-w-[1400px] mx-auto px-4 pt-6 flex flex-col sm:flex-row items-center justify-between gap-2 text-[10px] text-[#8C8175]">
        <span>© {new Date().getFullYear()} کالای خواب نفیس گلپایگان</span>
