  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        // FIX: Always update price range when data comes in
        if (data.priceRange?.max > 0) {
      ...
      <main className="flex-1">
        ...
        <div className="bg-white border-b border-gray-100">
          <div className="max-w-[1400px] mx-auto px-4 py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <h1 className="text-base sm:text-lg font-black">{CATS.find(c => c.id === cat)?.name || "همه محصولات نفیس"}</h1>
            </div>
