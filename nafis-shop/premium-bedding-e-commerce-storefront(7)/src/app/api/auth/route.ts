...
import { checkRateLimit, getClientIp, RL_AUTH_CHECK, RL_LOGIN } from "@/lib/rate-limit";

export const dynamic = "force-dynamic";

// Check if a phone number is already registered
// FIX: Rate-limited to prevent enumeration attacks
export async function GET(request: Request) {
  try {
    const ip = getClientIp(request);
    const rl = checkRateLimit(ip, RL_AUTH_CHECK);
    if (!rl.allowed) {
      return NextResponse.json(
        { error: "تعداد درخواست‌ها بیش از حد مجاز. لحظاتی صبر کنید." },
...
export async function POST(request: Request) {
  try {
    // FIX: Rate-limit login/register attempts
    const ip = getClientIp(request);
    const rl = checkRateLimit(ip, RL_LOGIN);
    if (!rl.allowed) {
      return NextResponse.json(
