  ...
  if (!product) return (
    <div className="min-h-screen flex flex-col bg-[#EBE4D8]">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
        <div className="text-6xl font-black text-[#7A8E72]/30 mb-3">۴۰۴</div>
        <h1 className="text-xl font-black text-[#3B3228] mb-2">محصول مورد نظر یافت نشد!</h1>
        <p className="text-sm text-[#8C8175] mb-6">این محصول حذف شده یا آدرسش اشتباهه.</p>
        <Link href="/shop" className="bg-[#3B3228] hover:bg-[#6B4226] text-white font-bold px-6 py-3 rounded-xl text-sm transition">بازگشت به فروشگاه</Link>
      </div>
      <Footer />
    </div>
  );
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#EBE4D8] text-[#3B3228]">
      ...
      <main className="flex-1 w-full">
        {/* Breadcrumb */}
        <Breadcrumb items={[
          { label: product.category || "محصولات", href: `/shop?category=${product.categorySlug}` },
          { label: product.namePersian || "جزئیات محصول" }
        ]} />
                ...
                {Array.isArray(product.availableColors) && product.availableColors.length > 0 && (
                  <div className="mt-3">
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-xs font-bold text-[#3B3228]">رنگ:</span>
                      {selectedColor && (
                        <span className="flex items-center gap-1.5 bg-[#F5F0E8] px-2 py-0.5 rounded-full text-xs font-bold text-[#3B3228]">
                          <span className="w-3 h-3 rounded-full border border-stone-300" style={{ backgroundColor: selectedColor.hex }} />
                          {selectedColor.name}
                        </span>
                      )}
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {product.availableColors.map((col: any, i: number) => (
                        <button key={i} onClick={() => { setSelectedColor(col); setColorChanged(true); }}
                          className={`w-7 h-7 rounded-full border-2 transition-all duration-200 ${selectedColor?.name === col.name ? "border-[#3B3228] scale-125 shadow-md" : "border-stone-200 hover:scale-105 hover:shadow-sm"}`}
                          style={{ backgroundColor: col.hex }} title={col.name} />
                      ))}
                    </div>
                    {colorChanged && selectedColor && (
                      <p className="text-[10px] text-[#7A8E72] mt-1.5 font-bold">
                        ✅ رنگ «{selectedColor.name}» انتخاب شد — تصویر اصلی نزدیک‌ترین نمایه به این رنگ هست
                      </p>
                    )}
                  </div>
                )}
            ...
            {activeTab === "reviews" && (
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                <div className="lg:col-span-7 space-y-3">
                  {reviews.length === 0 ? (
                    <div className="bg-[#F5F0E8] p-6 rounded-xl text-center space-y-2">
                      <MessageSquare className="w-8 h-8 text-[#7A8E72]/40 mx-auto" />
                      <p className="text-sm font-bold text-stone-600">هنوز کسی نظری ثبت نکرده!</p>
                      <p className="text-xs text-stone-500">اولین نفری باش که نظرت رو می‌نویسه — بقیه به تجربه‌ت نیاز دارن 😊</p>
                    </div>
                  ) : reviews.map((rev, i) => (
