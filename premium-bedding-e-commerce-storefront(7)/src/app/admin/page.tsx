...
import {
  Package, Plus, Trash2, Edit, TrendingUp, ShoppingBag, Clock,
...
const TOKEN_KEY = "nafis_admin_auth_token";
const TOKEN_VALUE = "nafis_secret_admin_token_2026";
...
export default function AdminPage() {
  ...
  const fetchData = useCallback(async () => {
    setDataLoading(true);
    try {
      const [pRes, oRes] = await Promise.all([fetch("/api/admin/products"), fetch("/api/admin/orders")]);
  ...
  useEffect(() => { try { if (localStorage.getItem(TOKEN_KEY) === TOKEN_VALUE) setAuthed(true); } catch {} setCheckingAuth(false); }, []);
  ...
  const doLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) { try { localStorage.setItem(TOKEN_KEY, TOKEN_VALUE); } catch {} setAuthed(true); }
      else setLoginErr(data.error || "خطا");
    } catch { setLoginErr("خطا"); } finally { setLoginLoading(false); }
  };

  const doLogout = () => { try { localStorage.removeItem(TOKEN_KEY); } catch {} try { fetch("/api/admin/auth", { method: "DELETE" }); } catch {} setAuthed(false); };

  const openAdd = () => {
    setEditing(false); setFId(null); setFName(""); setFCat("bedding-sets");
    setFPrice("4800000"); setFOrigPrice(""); setFDesc(""); setFFeatures("");
    setFSizes(DEFAULT_SIZES["bedding-sets"] || ""); setFImages(""); setFColors([]);
  ...
  const openEdit = (p: any) => {
    ...
    setFCat(p.categorySlug || "bedding-sets"); setFPrice(String(p.price || ""));
  ...
  if (!authed) return (
    <div className="min-h-screen bg-[#F5F0E8] flex items-center justify-center p-4">
      <div className="bg-white p-8 rounded-2xl shadow-lg max-w-sm w-full space-y-5">
        <div className="text-center space-y-2"><div className="w-12 h-12 rounded-xl bg-[#3B3228] text-white flex items-center justify-center mx-auto"><Lock className="w-6 h-6" /></div><h1 className="text-lg font-black">ورود به پنل مدیریت</h1></div>
  ...
  const filteredOrds = orders.filter(o => orderFilter === "all" || o.status === orderFilter);
      ...
      <div className="bg-white border-b border-[#EBE4D8]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex gap-1 overflow-x-auto scrollbar-none">
          {([
            ...
            { id: "ords" as const, icon: <ShoppingBag className="w-4 h-4" />, label: `سفارشات (${toPersianDigits(orders.length)})` },
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-16 overflow-y-auto">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl p-6 max-w-xl w-full z-10 space-y-4 mb-8">
            ...
            <form onSubmit={saveProd} className="space-y-3">
              ...
              <div><label className="text-xs font-bold block mb-1">توضیحات *</label><textarea required rows={2} value={fDesc} onChange={e => setFDesc(e.target.value)} className="w-full bg-[#F5F0E8] border border-[#EBE4D8] rounded-lg px-3 py-2.5 text-xs focus:outline-none focus:ring-1 focus:ring-[#7A8E72]" /></div>
              ...
              {/* Multi-image */}
              <div>
                <label className="text-xs font-bold block mb-1 flex items-center gap-1"><ImageIcon className="w-3.5 h-3.5" />تصاویر محصول (هر خط یک URL)</label>
                <textarea rows={3} value={fImages} onChange={e => setFImages(e.target.value)} placeholder={"https://example.com/img1.jpg\nhttps://example.com/img2.jpg\nhttps://example.com/img3.jpg"} className="w-full bg-[#F5F0E8] border border-[#EBE4D8] rounded-lg px-3 py-2.5 text-xs focus:outline-none focus:ring-1 focus:ring-[#7A8E72] dir-ltr text-left" />
