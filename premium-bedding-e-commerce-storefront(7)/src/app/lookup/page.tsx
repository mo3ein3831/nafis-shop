...
export default function LookupPage() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setSearched(true);
    try {
      if (searchTerm.trim().toUpperCase().startsWith("NFS-")) {
        const res = await fetch(`/api/orders/${searchTerm.trim().toUpperCase()}`);
        ...
        const res = await fetch(`/api/orders?phone=${encodeURIComponent(searchTerm.trim())}`);
        const data = await res.json();
        if (data.orders) {
      ...
      <main className="flex-1 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 w-full">
        <div className="text-center max-w-2xl mx-auto mb-8 space-y-2.5">
          ...
          <p className="text-xs sm:text-sm text-stone-600 leading-relaxed">
            برای اطلاع از وضعیت آماده‌سازی، ارسال و جزئیات فاکتور، شماره سفارش (مانند NFS-84910) یا شماره موبایلی که با آن ثبت سفارش کرده‌اید را وارد کنید.
          </p>
        </div>
          ...
          {!loading && searched && orders.length === 0 && (
            <div className="bg-white rounded-3xl p-10 text-center border border-[#EBE4D8] shadow-2xs space-y-3 max-w-xl mx-auto">
              ...
              <p className="text-xs text-stone-500 leading-relaxed">
                لطفاً از صحت شماره سفارش یا موبایل واردشده اطمینان حاصل کرده و مجدداً تلاش نمایید. در صورت نیاز با پشتیبانی گلپایگان تماس بگیرید (۰۳۱-۵۷۴۵۴۷۳۹).
              </p>
            </div>
          )}
