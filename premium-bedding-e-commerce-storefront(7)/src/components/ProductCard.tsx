...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <div className="group bg-white rounded-2xl overflow-hidden hover:shadow-xl transition-all duration-500 flex flex-col relative hover:-translate-y-1">
      {disc && disc > 0 && <span className="absolute top-2.5 right-2.5 z-10 bg-[#A0522D] text-white text-[10px] font-black px-2 py-0.5 rounded-lg">{toPersianDigits(disc)}٪</span>}
      {product.isNewArrival && <span className="absolute top-2.5 right-2.5 z-10 bg-[#5C6F54] text-white text-[9px] font-bold px-2 py-0.5 rounded-lg">{disc ? "" : "جدید"}</span>}
      ...
      <Link href={`/products/${product.slug}`} className="block aspect-square overflow-hidden bg-[#F5F0E8]">
        <img src={img} alt={product.namePersian} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
      </Link>
